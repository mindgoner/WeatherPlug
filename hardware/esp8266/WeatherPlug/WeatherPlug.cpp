...
WeatherPlug::WeatherPlug(const char* ssid, const char* password, const char* apiKey, const char* domain) {
  ...
  this->endpointPath = "/api/v1/data-receiver";
  this->updateExchangeUrl();
  WiFiMulti.addAP(this->ssid, this->password);

}
void WeatherPlug::updateExchangeUrl(){
  this->exchangeUrl = "http://"+String(domain)+String(this->endpointPath)+"?deviceMac="+String(this->getMACAddress())+"&temperature="+String(this->environmentTemperature)+"&humidity="+String(this->environmentHumidity)+"&pressure="+String(this->atmosphericPressure);  
}
